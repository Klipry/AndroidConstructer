...
class CategoryButtonColor(private val context: Context, private val logoValue: String) : AsyncTask<Void, Void, Boolean>() {
    ...
    override fun doInBackground(vararg params: Void?): Boolean {
        ...
        try {
            ...
            writer.write("arb=category_button_color") // Здесь мы отправляем параметр column_name со значением "logo_location"
            writer.flush()
            ...
            val logoLocation = try {
                jsonResponse.getString("category_button_color")
            } catch (e: JSONException) {
            ...
            val values = ContentValues().apply {
                put(DBHelper.KEY_CATEGORY_BUTTON_COLOR, logoLocation)
            }
            val selection = "${DBHelper.KEY_CATEGORY_BUTTON_COLOR} = ?"
