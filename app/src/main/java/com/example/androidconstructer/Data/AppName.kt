...
class AppName(private val context: Context, private val logoValue: String) : AsyncTask<Void, Void, Boolean>() {
    ...
    override fun doInBackground(vararg params: Void?): Boolean {
        ...
        try {
            ...
            writer.write("arb=app_name") // Здесь мы отправляем параметр column_name со значением "logo_location"
            writer.flush()
            ...
            val logoLocation = try {
                jsonResponse.getString("app_name")
            } catch (e: JSONException) {
            ...
            val values = ContentValues().apply {
                put(DBHelper.KEY_APP_NAME, logoLocation)
            }
            val selection = "${DBHelper.KEY_APP_NAME} = ?"
            val rowsAffected = db.update(DBHelper.TABLE_SETTING, values, null, null)
            if (rowsAffected == 0) {
                Log.e(TAG, "Failed to update app_name in local database")
                return false
            }
            ...
            Log.e(TAG, "Error updating app_name", e)
            return false
        }
    }
