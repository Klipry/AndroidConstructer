...
class BattonCategory3(private val context: Context, private val logoValue: String) : AsyncTask<Void, Void, Boolean>() {
    private var listener: (() -> Unit)? = null
    private val TAG = "UpdateCategory3"
    ...
    override fun doInBackground(vararg params: Void?): Boolean {
        ...
        try {
            ...
            writer.write("arb=ThreeCategory") // Здесь мы отправляем параметр column_name со значением "
            writer.flush()
            ...
            val logoLocation = try {
                jsonResponse.getString("ThreeCategory")
            } catch (e: JSONException) {
            ...
            val values = ContentValues().apply {
                put(DBHelper.ThreeCategory, logoLocation)
            }
