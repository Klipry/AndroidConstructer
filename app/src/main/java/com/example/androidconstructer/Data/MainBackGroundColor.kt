...
class MainBackGroundColor(private val context: Context, private val logoValue: String) : AsyncTask<Void, Void, Boolean>() {
    ...
    override fun doInBackground(vararg params: Void?): Boolean {
        ...
        try {
            ...
            writer.write("arb=main_activity_background_color") // Здесь мы отправляем параметр column_name со значением "logo_location"
            writer.flush()
            ...
            val logoLocation = try {
                jsonResponse.getString("main_activity_background_color")
            } catch (e: JSONException) {
            ...
            val values = ContentValues().apply {
                put(DBHelper.KEY_MAIN_ACTIVITY_BACKGROUND_COLOR, logoLocation)
            }
            val selection = "${DBHelper.KEY_LOGO_LOCATION} = ?"
            val rowsAffected = db.update(DBHelper.TABLE_SETTING, values, null, null)
            if (rowsAffected == 0) {
                Log.e(TAG, "Failed to update BACKGROUND in local database")
                return false
            }
